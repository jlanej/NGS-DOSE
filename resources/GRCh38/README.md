...
| `sinks.bed` | where the aligner puts each class's reads (fetch mode retrieves only these); since 2026-09-22 also the telomeric repeat's (`TEL`, 63 intervals, 810 kb, learned from 372 cohort scans, capture >= 99.67%), so a fetch with `resources/experimental/telomere.k31.panel.tsv.gz` loaded measures it | `ngsdose sinks` on whole-file scans of NA12878 (CEU, female) and HG02258 (ACB, male), NYGC pipeline (bwa-mem 0.7.15, ALT-aware); 80 intervals, 3.3 Mb, capturing ≥ 99.94% of 45S, 99.98% of 5S and 99.77% of DJ reads in both, and 99.96 / 99.99 / 99.75% in a third sample they never saw (HG01884). Learned on a 10-kb placement grid; scans now record 1 kb, on which the cohort run re-learns them tighter |
| `anchors.json` | the 45S windows that set the absolute level | windows on which the pilot's three sequencing chemistries agree (NovaSeq against HiSeq 2500 and HiSeq 2000), from its replicate pairs (`example/1000G/pilot/evaluate_pilot.py --write-anchors`) |
