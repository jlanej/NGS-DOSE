...
ngsdose_py ngsdose cohort "$EST_DIR"/*.estimate.json.gz -r "$BUNDLE" -t "$OUT/cohort.tsv" --save-efficiencies "$OUT/efficiencies.json" --control-pcs 10
...
# two adjustments: NGS-PCA's coverage PCs, and the ten internal control-region PCs from `cohort`
ngsdose_py ngsdose adjust "$OUT/cohort.tsv" --pcs "$NGSPCA_DIR/svd.pcs.txt" --n-pc 20 -c $COLS -o "$OUT/cohort.adj_ngspca.tsv"
ngsdose_py ngsdose adjust "$OUT/cohort.tsv" --n-pc 10 -c $COLS -o "$OUT/cohort.adj_ctrlpc.tsv"
